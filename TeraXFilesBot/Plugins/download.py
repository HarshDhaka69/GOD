...
async def progress_cbq(_, q):
    ...
    current = downloads_manager[user_id]['downloaded']
    total = downloads_manager[user_id]['size']
    status = 'Downloading' if downloads_manager[user_id]['method'] == 'd' else 'Uploading'
    
    await q.answer(
        f"{status}, {format_size(current)} / {format_size(total)}",
        show_alert=True
    )
